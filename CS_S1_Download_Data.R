...
for(i in c(country_set$iso2c)[! country_set$iso2c %in% countries_leftout]){
  ...
  dots <- imf_data(database_id = "DOT", 
                   indicator = "TXG_FOB_USD", 
                   country = i, 
                   freq = "A", 
                   start = start_year,
                   end = end_year,
                   return_raw = TRUE)$CompactData$DataSet$Series %>%
    filter(`@FREQ` == "A", `@COUNTERPART_AREA` %in% c(country_set$iso2c)) %>%
    mutate(Obs = map(Obs, as.data.frame)) %>%
    mutate(Obs = map(Obs, ~ rename_with(., ~ ifelse(grepl("TIME", .), "@TIME_PERIOD", .)))) %>%
    mutate(Obs = map(Obs, ~ rename_with(., ~ ifelse(grepl("VALUE", .), "@OBS_VALUE", .)))) %>%
    unnest(Obs) %>%
    #unnest_wider(Obs) %>%
    #unnest(c(`@TIME_PERIOD`, `@OBS_VALUE`)) %>%
    mutate(`@REF_AREA` = as.character(`@REF_AREA`)) %>%
    replace_na(list(`@REF_AREA` = "NA")) %>%
    mutate(`@OBS_VALUE` = as.numeric(`@OBS_VALUE`), `@UNIT_MULT` = as.numeric(`@UNIT_MULT`)) %>%
    mutate(`@OBS_VALUE_2` = `@OBS_VALUE`*10^(`@UNIT_MULT`)) %>%
    select(c(`@REF_AREA`,`@COUNTERPART_AREA`,`@TIME_PERIOD`,`@OBS_VALUE_2`))
  
  imf_dots_base <- rbind(imf_dots_base, dots)
  countries_leftout <- c(countries_leftout,i)
  rm(dots)
}
...
for(i in c(country_set$iso2c)[! country_set$iso2c %in% countries_leftout_tw]){
  ...
  dots <- imf_data(database_id = "DOT", 
    ...
    mutate(`@REF_AREA` = as.character(`@REF_AREA`)) %>%
    replace_na(list(`@REF_AREA` = "NA")) %>%
    filter(`@FREQ` == "A", `@COUNTERPART_AREA` == "TW") %>%
    mutate(`@OBS_VALUE` = as.numeric(`@OBS_VALUE`), `@UNIT_MULT` = as.numeric(`@UNIT_MULT`)) %>%
    mutate(`@OBS_VALUE_2` = `@OBS_VALUE`*10^(`@UNIT_MULT`)) %>%
    select(c(`@REF_AREA`,`@COUNTERPART_AREA`,`@TIME_PERIOD`,`@OBS_VALUE_2`))
...
imf_dots <- tibble()
imf_dots <- rbind(imf_dots_base, imf_dots_tw)

#Inflation adjustment-----

##Import USA GDP Deflator and set 100 to latest year----
usa_gdp_deflator <- imf_data(database_id = "IFS",
                             indicator = "NGDP_D_IX",
                             country ="US",
                             freq = "A",
                             start = start_year,
  ...
  as_tibble() %>% 
  mutate(deflator = NGDP_D_IX/filter(.,year == end_year)$NGDP_D_IX*100)
